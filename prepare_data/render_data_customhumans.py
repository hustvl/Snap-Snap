...
def apply_transformations(obj, trans, orient):
    # Apply translation
    translation_matrix = np.eye(4)
    translation_matrix[:3, 3] = trans

    # Apply rotation (convert axis-angle to rotation matrix)
    rotation = R.from_rotvec(orient)
    ...
    # Apply the transformation to the model's vertices
    ones = np.ones((obj['vi'].shape[0], 1))
    vertices_homogeneous = np.hstack((obj['vi'], ones))
    transformed_vertices = np.dot(transformation_matrix.T, vertices_homogeneous.T).T

    # Update the model's vertices
    obj['vi'] = transformed_vertices[:, :3]

    
def render_data(renderer, data_path, data_id, save_path, phase, cam_settings, res, dis=1.0, is_thuman=False):
    obj_path = os.path.join(data_path, 'mesh', phase, 'mesh-f%s.obj' % phase.split('_')[-1])
    smpl_obj_path = os.path.join(data_path, 'smplx', phase, 'mesh-f%s_smpl.obj' % phase.split('_')[-1])
    texture_path = data_path
    img_path = os.path.join(texture_path, 'mesh', phase, 'mesh-f%s.png' % phase.split('_')[-1])
    ...
    human_height = 1.80 # + np.random.uniform(-0.05, 0.05, 1) # offset
    ...
    setting_interval = 360 / cam_settings
    angle_base = 0

    for pid in range(cam_settings):
        angle = angle_base + pid * setting_interval # degree_interval
        ...
        extr, intr, depth, img, mask, img_hr = render(dis, angle, look_at_center, base_cam_pitch, renderer, render_2k=True)
...
if __name__ == '__main__':
    ...
    scene_radius = 2
    res = (1024, 1024)
    thuman_root = '/data/lujia/CustomHumans'
    save_root = '/data/lujia/CustomHumans_30_val'

    renderer = StaticRenderer()
    thuman_list = sorted(os.listdir(os.path.join(thuman_root, 'mesh')))

    for data_id, phase in tqdm(enumerate(thuman_list)):
        
        data_id = '{:04d}'.format(data_id)
        render_data(renderer, thuman_root, data_id, save_root, phase, cam_settings, res, dis=scene_radius)


# CUDA_VISIBLE_DEVICES=0 python prepare_data/render_data_customhumans.py