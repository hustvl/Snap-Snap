...
def render(dis, angle, look_at_center, p, renderer, render_2k=False, noise=-1):
...
def render_data(renderer, data_path, data_id, save_path, cam_nums, res, dis=1.0):
    obj_path = os.path.join(data_path, data_id, f'{data_id}_textured.obj')
    img_path = os.path.join(data_path, data_id, f'{data_id}_texture.png')
    ...
    obj['vi'][:, 0] -= np.mean(obj['vi'][:, 0])
    obj['vi'][:, 2] -= np.mean(obj['vi'][:, 2])
    ...
    for pid in range(cam_nums):
        ...
        extr, intr, depth, img, mask = render(dis, angle, look_at_center, base_cam_pitch, renderer, render_2k=False)
...
if __name__ == '__main__':
    ...
    scene_radius = 2
    res = (1024, 1024)
    thuman_root = '/data/lujia/2k2k/textured_obj'
    save_root = '/data/lujia/render_120v/render_data_2k2k_1024_120_val' 
    ...
    for data_id in tqdm(thuman_list):
        save_path = save_root
        data_id = '{:05d}'.format(int(data_id))
